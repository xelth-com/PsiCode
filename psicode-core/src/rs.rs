//! Рид-Соломон над GF(32): два перемежённых кодовых слова RS(20, 12).
//!
//! Код профиля — 40 символов (200 бит). Одно RS-слово над GF(32) длиннее 31
//! символа невозможно, поэтому код собирается из двух: чётные позиции —
//! слово A, нечётные — слово B. Каждое слово: 12 информационных + 8
//! проверочных символов, исправляет до 4 ошибок в позициях, неизвестных
//! декодеру (ровно то, что нужно для опечаток при ручном вводе).
//!
//! Гарантии всего 40-символьного кода: любые <=4 опечатки; до 8 при
//! равномерном распределении; сплошная порча до 8 подряд идущих символов
//! (например, целая группа из 5) — перемежение делит её между A и B.
//!
//! Систематический код: первые 12 символов слова — сообщение как есть.
//! Корни порождающего полинома: alpha^0 .. alpha^7 (fcr = 0).

use crate::gf32 as gf;

pub const N: usize = 20; // длина одного кодового слова (максимум для GF(32): 31)
pub const K: usize = 12; // информационные символы одного слова
pub const NSYM: usize = N - K; // 8 проверочных
const T: usize = NSYM / 2; // исправляем до 4 ошибок на слово

pub const MSG_LEN: usize = 2 * K; // 24 информационных символа во всём коде
pub const CODE_LEN: usize = 2 * N; // 40 символов во всём коде
...
/// Кодирует 12 символов (значения 0..31) в одно слово из 20 символов.
...
/// Кодирует 24 информационных символа в 40: слово A из первых 12, слово B из
/// последних 12, перемежение A0 B0 A1 B1 ...
...
/// Информационные символы из (исправленного) 40-символьного кода.
