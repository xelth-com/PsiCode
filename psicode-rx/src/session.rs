...
/// Диапазон поиска КАМЕРНЫХ px на клетку для корреляционного захвата рамки v1
/// (§3.2). Верхняя граница — «символ во весь кадр».
///
/// # Нижняя граница: 5, а не 7
///
/// Прежние 7 объяснялись «порогом читаемости ~6 px/клетку». Замер обе половины
/// этого утверждения не подтверждает, причём в разные стороны:
///
/// * **Захват** держится гораздо ниже: `psicode-core --example pxsweep` при
///   расфокусе σ = 2 камерных px (замеренный A22) принимает рамку с **3.5**
///   камерных px/клетку, промах углов при этом 0.09 клетки — то есть точность
///   геометрии в ДОЛЯХ КЛЕТКИ на мелкой клетке не деградирует. Ограничивает не
///   захват.
/// * **Нагрузка** ограничивает раньше захвата. `psicode-sim cellfloor floor`
///   (доля дошедших payload-бит с учётом выживания страйпов §6.2):
///
///   | камерных px/клетку | 4.2 | 5.3 | 6.4 | 7.4 | 8.5 |
///   |---|---|---|---|---|---|
///   | 1 бит яркости, ISI выкл | 0.27 | 0.87 | 0.93 | 0.99 | 1.00 |
///   | 1 бит яркости, ISI вкл | 0.38 | 0.88 | 0.99 | 1.00 | 1.00 |
///   | §5.1-CL 2 бита, ISI вкл | 0.00 | 0.00 | 0.14 | 0.51 | 0.85 |
///
/// Отсюда 5.0: на 5.3 яркостная нагрузка ещё отдаёт 0.87 полезных бит, на 4.2
/// уже 0.27 — искать там значит ловить символ, который всё равно не прочесть.
/// (§5.1-CL требует заметно больше — от ~9; но диапазон один на оба режима, и
/// правит его более живучий.)
///
/// # Цена
///
/// Лестница масштабов геометрическая, шаг `SCALE_RATIO` = 1.12, поэтому ступеней
/// `log(20/низ)/log(1.12)`: 10 при 7.0 и 13 при 5.0, +30 %. Замер холодного
/// захвата (`pxsweep <σ> cost`, один поток, сцена 10.5 px/клетку) даёт +20…25 %
/// ко времени — меньше числа ступеней, потому что затравочный скан лишь часть
/// стоимости. Платится ТОЛЬКО на холодном захвате: после лока работает
/// `acquire::track`, который диапазон масштабов не перебирает.
const V1_PX_PER_CELL: (f64, f64) = (5.0, 20.0);
