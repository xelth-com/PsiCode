...
/// Наименьший размер клетки ПОКАЗА в px. Вместо обрезки символа при слишком
/// маленьком окне мы отказываемся показывать (§bug).
///
/// # Почему 4, а не 8
///
/// Прежние 8 ссылались на `symbol::sample_cell` («ниже 8 теряется 2×2
/// субсэмплинг»). Ссылка была неверна ПО ПОСТРОЕНИЮ: `sample_cell` смотрит на
/// `profile.cell_size_px` — величину ПРИЁМНИКА, у которого она всегда 16, — а не
/// на клетку показа передатчика. Условие не зависело от того, что ограничивало.
///
/// Что ограничивает на самом деле — КАМЕРНЫХ px на клетку, а это свойство
/// стенда (расстояние, объектив, сенсор), которое передатчик не знает и знать не
/// может. Клетка показа входит в дело только через апертуру пикселя ПАНЕЛИ
/// (≈ 1 display px): чем меньше клетка, тем большую её долю занимает пиксель.
///
/// Замер (`psicode-sim cellfloor sep`, рамка v1, замеренный тракт A22): при
/// ФИКСИРОВАННЫХ камерных px/клетку доля кадров с восемью живыми страйпами по
/// клетке показа 3…16 плоская в пределах статистики — и в 1-битном яркостном
/// режиме, и в 2-битном §5.1-CL, с выравнивателем ISI и без. Точечно, на
/// маргинальном канале (6.0 камерных px/клетку, 160 попыток, ±0.03):
///
/// | клетка показа | 1 | 2 | 3 | 4 | 6 |
/// |---|---|---|---|---|---|
/// | кадров 8/8 | 0.03 | 0.67 | 0.78 | 0.81 | 0.86 |
///
/// На канале с запасом (10.5 камерных px/клетку) клетки 2…12 дают ровно 1.000, а
/// клетка 1 — 0.79: при ней клетка И ЕСТЬ один пиксель панели, и её апертура
/// съедает символ целиком.
///
/// Отсюда 4 — наименьшая клетка показа, чья цена НЕОТЛИЧИМА от нуля на любом
/// замеренном канале (−0.06 при σ 0.03 даже на маргинальном). 3 и 2 всё ещё
/// рисуются верно и читаются, когда у канала есть запас, но платят измеримо; 1
/// не работает. Модель к тому же КОНСЕРВАТИВНА: апертура панели в ней — билинейный
/// шатёр шириной 2 display px, тогда как у настоящей панели это бокс ≤ 1 px.
const MIN_DISPLAY_CELL: usize = 4;
                ...
                if !self.printed_too_small {
                    ...
                    eprintln!(
                        "Окно слишком мало: клиентская область {win_w}×{win_h} px. \
                         ...
                         клетке показа {MIN_DISPLAY_CELL} px (замер: ниже неё апертура \
                         пикселя панели начинает съедать клетку) нужна клиентская \
                         область не меньше {need}×{need} px. Увеличьте окно и \
                         перезапустите.\n\
                         Учтите: читаемость определяет НЕ клетка показа, а КАМЕРНЫХ \
                         px на клетку у приёмника (свойство расстояния и объектива). \
                         Приёмник ищет символ в диапазоне 5..20 камерных px/клетку; \
                         1-битной яркости хватает примерно от 6, §5.1-CL — от 9."
                    );
                }
...
mod tests {
    ...
    /// 2b: отказ ниже пола. Лучше явный отказ, чем обрезанный символ.
    #[test]
    fn max_display_cell_refuses_below_floor() {
        // ровно на полу: 4·69 = 276 помещается -> Some(4)
        assert_eq!(max_display_cell(276, 276, 69, MIN_DISPLAY_CELL), Some(4));
        // на 1 px меньше: 275/69 = 3 < 4 -> None (отказ, НЕ обрезаем)
        assert_eq!(max_display_cell(275, 275, 69, MIN_DISPLAY_CELL), None);
        // крохотное окно -> None
        assert_eq!(max_display_cell(200, 200, 69, MIN_DISPLAY_CELL), None);
        // v1-пол: 4·61 = 244 -> Some(4); 243 -> None
        assert_eq!(max_display_cell(244, 244, 61, MIN_DISPLAY_CELL), Some(4));
        assert_eq!(max_display_cell(243, 243, 61, MIN_DISPLAY_CELL), None);
    ...
    /// Пол клетки показа ПРИВЯЗАН К ЗАМЕРУ, а не к круглому числу.
    ///
    /// Замер `psicode-sim cellfloor` (рамка v1, замеренный тракт A22, доля
    /// кадров с восемью живыми страйпами) при ФИКСИРОВАННЫХ камерных px/клетку —
    /// то есть при вкладе ОДНОЙ ЛИШЬ клетки показа:
    ///
    /// | клетка показа | 1 | 2 | 3 | 4 | 6 |
    /// |---|---|---|---|---|---|
    /// | 6.0 камерных px/кл, 160 попыток | 0.03 | 0.67 | 0.78 | 0.81 | 0.86 |
    /// | 10.5 камерных px/кл, 48 попыток | 0.79 | 1.00 | 1.00 | 1.00 | 1.00 |
    ///
    /// Пол = наименьшая клетка, чья просадка на МАРГИНАЛЬНОМ канале лежит в
    /// пределах двух стандартных ошибок (σ ≈ 0.03 при 160 попытках) от полки
    /// крупных клеток. Тест ловит два вырождения: возврат пола к 8 (запрещает
    /// вчетверо больше клеток на том же экране, без единого замера в оправдание)
    /// и сползание к 1..2 (там просадка уже НЕ в пределах статистики).
    #[test]
    fn display_cell_floor_matches_measurement() {
        /// (клетка показа, доля кадров 8/8 при 6.0 камерных px/клетку)
        const MARGINAL: [(usize, f64); 5] =
            [(1, 0.031), (2, 0.669), (3, 0.775), (4, 0.806), (6, 0.863)];
        /// Стандартная ошибка доли при 160 попытках, p ≈ 0.8.
        const SE: f64 = 0.032;
        let plateau = MARGINAL.last().unwrap().1;

        // пол ДОПУСТИМ: его просадка от полки не больше 2σ
        let at_floor = MARGINAL
            .iter()
            .find(|(c, _)| *c == MIN_DISPLAY_CELL)
            .expect("пол должен быть среди замеренных клеток");
        assert!(
            plateau - at_floor.1 <= 2.0 * SE,
            "пол {MIN_DISPLAY_CELL}: просадка {:.3} больше 2σ = {:.3}",
            plateau - at_floor.1,
            2.0 * SE
        );
        // ниже пола просадка УЖЕ выходит за 2σ — то есть пол не занижен
        for (c, full) in MARGINAL {
            if c < MIN_DISPLAY_CELL {
                assert!(
                    plateau - full > 2.0 * SE,
                    "клетка {c} не хуже пола на 2σ — пол завышен"
                );
            }
        }
        // и не завышен: 8 (прежнее значение) не оправдано ни одним замером —
        // разница 4 -> 8 это вчетверо больше клеток на том же экране.
        assert!(MIN_DISPLAY_CELL <= 4, "пол выше замеренного минимума");
    }
